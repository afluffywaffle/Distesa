...
## Thread: phase1
_Updated 2026-07-18 (session R)_

### Session 2026-07-18 (R): VIEW/SEND intent filters + onNewSession fix — COMMITTED `995e638`, device-VERIFIED
Roadmap item ② from session Q. `AndroidManifest.xml`: MainActivity now `launchMode="singleTask"` + ACTION_VIEW (http/https, BROWSABLE) + ACTION_SEND (text/plain) filters. `MainActivity.kt`: new `urlFromIntent()` helper (navUrl extra → VIEW `intent.data` → first http(s) URL in SEND `EXTRA_TEXT`) used by BOTH `onCreate` and `onNewIntent`; `onNewSession` override on `EtpNavigationDelegate` loads `_blank`/`window.open` targets into the existing single session and returns `GeckoResult.fromValue(null)` (tabs are OUT of the roadmap — user call, e-ink-native single-session model). Opus gate PASS. **Verified on the Nomad:** fresh VIEW launch (example.com), onNewIntent re-delivery (httpbin.org), SEND URL-extraction (example.org) — all loaded per `[eink-perf]`. NOTE: an untargeted `am start -a VIEW` resolves to another installed browser; use `-p com.afluffywaffle.avosetta.dev`. `onNewSession` not human-driven yet (needs a real `_blank` tap) but code-reviewed. **This unlocks adb-driven page loads** — plain `am start -a android.intent.action.VIEW -d <URL> -p com.afluffywaffle.avosetta.dev` now works (old `--es navUrl` trick still does too).
**NEXT: roadmap item ③ — the NOTEPAD (split-view scratchpad). Big build, needs its own plan first.** Settled v1 decisions (session Q discussion; no plan file exists): fixed 50/50 split (web top / pad bottom in portrait; pad on handedness side in landscape), half-size rails + none on the pad side, pen-vs-finger routing via `MotionEvent.getToolType` (palm rejection free), fixed logical canvas per page scale-to-fit (strokes never reflow), paginated pad (no scroll), notes URL-bound to source page, per-note JSON (manifest + per-page stroke lists), export = flattened PDF or PDF with vector Ink annotations, hub "Notes" list sibling to History. After: ④ reader mode (later, independent track). Browser-action popup / Raindrop: RETIRED (user call, not feasible). Tabs: RETIRED.
